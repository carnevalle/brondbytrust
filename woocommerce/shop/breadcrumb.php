...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	echo $wrap_before . $before  . '<a class="home" href="' . $home_link . '">' . $home . '</a> ' . $delimiter . $after;
	...
	if ( is_category() ) {
		...
		if ( ! empty( $parents ) ) {
			...
			foreach ( $parents as $parent ) {
				...
				echo $before .  '<a href="' . get_term_link( $item->slug, 'product_cat' ) . '">' . $item->name . '</a>' . $delimiter  . $after;
			}
		}
		...
		if ( get_post_type() == 'product' ) {
			...
			if ( $terms = wp_get_object_terms( $post->ID, 'product_cat' ) ) {
				...
				echo $before . '<a href="' . get_term_link( $term->slug, 'product_cat' ) . '">' . $term->name . '</a>' . $delimiter . $after;
